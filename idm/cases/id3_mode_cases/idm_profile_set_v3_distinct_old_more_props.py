...
class IdmProfileSetV3DistinctOldUserMorePropsCase(TestCase):
    ...
    def do_test(self, servers, count, list_count, proportion=0):
        ...
        gc.collect()
        print("导入老用户 profile_set(125 个属性 version=3.0) 数据完成")
    ...
    def do_import_test(self, exec_ip, project_name, count, import_mode):
        ...
        with open(self.import_file_name, "w") as f:
            for i in range(0, count):
                ret = self.make_profile_set_v3_more_old_user(1, already_identities)
    ...
    def collect_import_qps(self, count):
        qps_detail = {}
        qps_detail['title'] = "profile_set 老用户(125个属性, 属性发生变更)-importer"
