...
class MockIdmProfileSetDistinctNewUserMorePropsCase(TestCase):
    ...
    def do_test(self, servers, count, list_count, proportion=0):
        ...
        print("导入 profile_set(Mock IDM, 匿名用户 125个属性 version=2.0) 数据完成, 数据量={}".format(total_count))

    def run_make_records(self, servers, count, list_count, concurrent_index):
        print("导入 profile_set(Mock IDM, 匿名用户 125个属性 version=2.0) 数据, 并发序号={}, 导入量={}".format(concurrent_index, count))
