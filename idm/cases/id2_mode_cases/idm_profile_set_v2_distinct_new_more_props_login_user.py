...
class IdmProfileSetV2DistinctNewLoginUserMorePropsCase(TestCase):

    def __init__(self, build_user, identification):
        super().__init__()
        self.file_name = "profile_set_v2_more_{}_{}.json".format(build_user, identification)
        self.import_file_name = "IdmProfileSetV2DistinctNewLoginUserMorePropsCase_{}_{}_importer.json".format(
            build_user, identification)
        self.cost = 0
    ...
    def do_test(self, servers, count, list_count, proportion=0):
        print("开始导入 profile_set(登录新用户 125 个属性 version=2.0) 数据, 数据量={}".format(count))
        ...
        print("导入 profile_set(登录新用户 125 个属性 version=2.0)  数据完成, 数据量={}".format(total_count))

    def run_make_records(self, servers, count, list_count, concurrent_index):
        print("导入 profile_set(登录新用户, 125 个属性 version=2.0) 数据, 并发序号={}, 导入量={}".format(concurrent_index, count))
        start = int(time.time())
        ...
        print("导入 profile_set(登录新用户, 125 个属性 version=2.0) 数据, 并发序号={}, 导入量={}, cost={}ms"
    ...
    def make_profile_set_v2_more(self, count):
        ...
        for i in range(count):
            ...
            device_id = str(uuid.uuid4()) + str(int(current_time * 1000000)) + '_' + str(random_num)
            login_id = 'user_' + str(int(current_time * 1000000)) + '_' + str(random_num)
            profile_set_identity_json['anonymous_id'] = device_id
            profile_set_identity_json['login_id'] = login_id
            profile_set_identity_json['distinct_id'] = login_id
            profile_set_identity_list.append(profile_set_identity_json)

            profile_set_json['anonymous_id'] = device_id
    ...
    def do_import_test(self, exec_ip, project_name, count, import_mode):
        ...
        with open(self.import_file_name, "w") as f:
            for i in range(0, count):
                ret = self.make_profile_set_v2_more(1)
    ...
    def collect_import_qps(self, count):
        qps_detail = {}
        qps_detail['title'] = "profile_set (登录新用户, 125个属性)-importer"
