...
class IdmProfileSetV2DistinctNewLoginUserCase(TestCase):
    ...
    def __init__(self, build_user, identification):
        ...
        self.input_file_dir = ""
        self.output_file_dir = "profile_set_v2_login_user_{}_{}".format(build_user, identification)
        self.cost = 0

    def do_test(self, exec_ip, ips, project, count):
        print("开始导入 profile_set(登录新用户 125 个属性 version=2.0) 数据, 数据量={}".format(count))
        start_spark_job(exec_ip, self.work_path, self.script_path, "IdmProfileSetV2DistinctNewLoginUserCase", 'id2',
