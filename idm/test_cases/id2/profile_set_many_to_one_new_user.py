...
class IdmProfileSetManyToOneUserCase(TestCase):
    ...
    def do_test(self, basic_data_ip, target_ips, project, count):
        print("开始导入 profile_set(多对一新用户 125 个属性 version=2.0) 数据, 数据量={}".format(count))
        start_spark_job(basic_data_ip, self.work_path, self.script_path,
                        "IdmProfileSetManyToOneUserCase", 'id2',
                        target_ips, project, self.basic_data_path, self.data_type)
        print("导入 profile_set(多对一新用户, 125 个属性 version=2.0) 数据完成, 数据量={}".format(count))

    def collect_qps(self, exec_ip, data_count):
        qps_detail = collect_sdi_qps(exec_ip, data_count)
        qps_detail['title'] = "profile_set (多对一新用户, 125个属性)"
        return qps_detail
