...
class IdmTrackV2DistinctOldAnonymousUserCase(TestCase):
    ...
    def do_test(self, exec_ip, ips, project, count):
        count = count * 3
        print("开始导入 track(匿名老用户 25 个属性 version=2.0) 数据, 数据量={}".format(count))
        start_spark_job(exec_ip, self.work_path, self.script_path, "IdmTrackV2DistinctOldAnonymousUserCase", 'id2', ips,
