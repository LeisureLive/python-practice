...
class IdmProfileSetV2DistinctOldAnonymousUserCase(TestCase):
    ...
    def do_test(self, exec_ip, ips, project, count):
        count = count * 3
        print("开始导入 profile_set(匿名老用户 125 个属性 version=2.0) 数据, 数据量={}".format(count))
        start_spark_job(exec_ip, self.work_path, self.script_path, "IdmProfileSetV2DistinctOldAnonymousUserCase", 'id2',
