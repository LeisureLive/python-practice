...
class IdmProfileSetV2DistinctOldLoginUserCase(TestCase):
    ...
    def __init__(self, idm_engine_type):
        ...
        self.basic_data_path = f"hdfs:///sa/runtime/daily_benchmark_basic_data/{idm_engine_type}/id2/login_old_profile_set"
    ...
    def do_test(self, basic_data_ip, target_ips, project, count):
        ...
        start_spark_job(basic_data_ip, self.work_path, self.script_path, "IdmProfileSetV2DistinctOldLoginUserCase",
                        'id2', target_ips, project, self.basic_data_path, self.data_type)
        print("导入 profile_set(登录老用户, 125个属性 version=2.0) 数据完成, 数据量={}".format(count))

    def collect_qps(self, exec_ip, data_count):
        qps_detail = collect_sdi_qps(exec_ip, data_count)
        qps_detail['title'] = "profile_set (登录老用户, 125个属性)"
        return qps_detail
