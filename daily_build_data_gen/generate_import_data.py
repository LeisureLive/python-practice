...
profile_set_generator = \
    ProfileSetGenerator(idm_version, user_count, login_percent, new_user_percent, exist_identities)
track_generator = TrackGenerator(idm_version, event_count, login_percent, new_user_percent, exist_identities)
...
if output_file_path != "hdfs:///sa/runtime/import_data_daily_benchmark":
    user_rdd \
        .map(profile_set_generator.filter_identiy_info) \
        .toDF() \
        .write.mode("overwrite").json(output_file_path)
else:
    user_rdd.mapPartitions(data_sender.batch_send_to_import_api).collect()
