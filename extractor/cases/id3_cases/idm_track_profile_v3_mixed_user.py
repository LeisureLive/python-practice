...
class IdmTrackProfileV3MixedUserCase(TestCase):
    ...
    def do_test(self, servers, count, list_count, proportion=0):
        print("开始导入新老用户 profile + track 混合数据(version=3.0), 数据量={}".format(count * 4))
        with open(self.file_name, 'r') as f:
            json_data = f.readlines()
        already_identities = [json.loads(line.strip()) for line in json_data]
        cnt = int(count * 4 / 1000)
        for i in range(cnt):
            test_data = self.make_track_profile_mixed_user(already_identities)
    ...
    def make_track_profile_mixed_user(self, already_identities):
        jsonStringV3 = []
        num = 1
        profile_count = 0
        track_count = 0
