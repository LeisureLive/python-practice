...
class IdmProfileSetV3DistinctNewUserMorePropsCase(TestCase):
    ...
    def __init__(self, build_user, identification):
        ...
        self.import_file_name = "IdmProfileSetV3DistinctNewUserMorePropsCase_{}_{}_importer.json".format(build_user,
                                                                                                         identification)
        self.cost = 0
        super().__init__()
    ...
    def do_test(self, servers, count, list_count, proportion=0):
        ...
        print("导入 profile_set(125 个属性 version=3.0) 数据完成, 数据量={}".format(total_count))
    ...
    def run_make_records(self, servers, count, list_count, concurrent_index):
        ...
        print("导入 profile_set(125 个属性 version=3.0) 数据, 并发序号={}, 导入量={}, cost={}ms"
    ...
    def do_import_test(self, exec_ip, project_name, count, import_mode):
        ...
        with open(self.import_file_name, "w") as f:
            for i in range(0, count):
                ret = self.make_profile_set_v3_more(1, 1)
    ...
    def collect_import_qps(self, count):
        qps_detail = {}
        qps_detail['title'] = "profile_set 新用户(125个属性)-importer"
